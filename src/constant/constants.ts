export const NAME_REQUIRED = "Name is required";
export const NAME_NOT_NULL = "Name cannot be null";
export const NAME_NOT_EMPTY = "Name cannot be empty";
export const ORIGINAL_URL_REQUIRED = "Original url is required";
export const ORIGINAL_URL_NOT_NULL = "Original url cannot be null";
export const ORIGINAL_URL_NOT_EMPTY = "Original url cannot be empty";
export const UNIQUE_URL_REQUIRED = "Unique url is required";
export const UNIQUE_URL_NOT_NULL = "Unique url cannot be null";
export const UNIQUE_URL_NOT_EMPTY = "Unique url cannot be empty";
export const HOME_TEAM_REQUIRED = "Home team is required";
export const HOME_TEAM_NOT_NULL = "Home team cannot be null";
export const HOME_TEAM_NOT_EMPTY = "Home team cannot be empty";
export const AWAY_TEAM_REQUIRED = "Away team is required";
export const AWAY_TEAM_NOT_NULL = "Away team cannot be null";
export const AWAY_TEAM_NOT_EMPTY = "Away team cannot be empty";
export const FIXTURE_DATE_REQUIRED = "Fixture date is required";
export const FIXTURE_DATE_NOT_NULL = "Fixture date cannot be null";
export const FIXTURE_DATE_NOT_EMPTY = "Fixture date cannot be empty";
export const STATUS_REQUIRED = "Status is required";
export const STATUS_NOT_NULL = "Status cannot be null";
export const STATUS_NOT_EMPTY = "Status cannot be empty";
export const PASSWORD_REQUIRED = "Password is required";
export const PASSWORD_NOT_NULL = "Password cannot be null";
export const PASSWORD_NOT_EMPTY = "Password cannot be empty";
export const UNIQUE_EMAIL = "Email already exist";
export const UNIQUE_TEAM = "Team with name already exist";
export const UNIQUE_EMAIL_CODE = "unique_email";
export const UNIQUE_URL_CODE = "unique_url";
export const UNIQUE_URL = "Unique url already exist";
export const UNIQUE_TEAM_CODE = "unique_team";
export const EMAIL_REQUIRED = "Email is required";
export const TEAM_NAME_REQUIRED = "Team name is required";
export const EMAIL_NOT_NULL = "Email cannot be null";
export const EMAIL_NOT_EMPTY = "Email cannot be empty";
export const TEAM_NAME_NOT_NULL = "Team name cannot be null";
export const TEAM_NAME_NOT_EMPTY = "Team name cannot be empty";
export const VALID_EMAIL = "Valid email required";
export const FIRST_NAME_REQUIRED = "First name is required";
export const FIRST_NAME_NOT_EMPTY = "First name cannot be empty";
export const FIRST_NAME_NOT_NULL = "First name cannot be null";
export const FIXTURE_ID_REQUIRED = "Fixture id is required";
export const FIXTURE_ID_NOT_EMPTY = "Fixture id cannot be empty";
export const FIXTURE_ID_NOT_NULL = "Fixture id cannot be null";
export const UNIQUE_CODE_REQUIRED = "Unique code is required";
export const UNIQUE_CODE_NOT_EMPTY = "Unique code cannot be empty";
export const UNIQUE_CODE_NOT_NULL = "Unique code cannot be null";
export const LAST_NAME_NOT_EMPTY = "Last name is required";
export const LAST_NAME_REQUIRED = "Last name is required";
export const LAST_NAME_NOT_NULL = "Last name cannot be null";
export const VALID_PASSWORD_LENGTH =
    "Your password must be at least 8 characters long";
export const MATCHING_PASSWORD = "Passwords do not match";
export const CONFIRM_PASSWORD_REQUIRED = "Confirm Password is required";
export const EMPTY_CONFIRM_PASSWORD = "Confirm password cannot be empty";
export const UNKNOWN_ERROR = "Unknown error occurred";
export const UNIQUE_ERROR = "SequelizeUniqueConstraintError";
export const WRONG_CREDENTIALS =
    "Incorrect email or password. Please double-check and try again";
export const AUTH_TOKEN_REQUIRED = "Access token is required for this route";
export const TOKEN_REQUIRED = "Token is required";
export const AUTH_TOKEN_EXPIRED =
    "Access token has expired, please login again";
export const TEAM_NOT_FOUND = "Team not found";
export const FIXTURE_NOT_FOUND = "Fixture not found";
export const INVALID_LINK = "Invalid link";
export const SEQUELIZE_DATABASE_ERROR = "SequelizeDatabaseError";
export const COMPLETED = "COMPLETED";
export const PENDING = "PENDING";
export const LIVE = "LIVE";
export const INVALID_MATCH_STATUS = "Invalid match status";
export const HOME_TEAM_NOT_FOUND = "Home team not found";
export const AWAY_TEAM_NOT_FOUND = "Away team not found";
export const SAME_TEAM = "Cant set fixtures between same team";
export const REDIS_LONG_TERM_CACHE_TIME = 604800; // 7 days
